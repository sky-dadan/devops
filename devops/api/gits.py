...
def git_create(auth_info, **kwargs):
    ...
    try:
        ...
        if not data.has_key('principal') and  not app.config['cursor'].if_id_exist('user',data['principal'].split(',')):         
        ...
        if r['code'] == 1:
            return json.dumps(r)
        # project_users = utils.project_members(db=app.config['cursor']).get(data['name'])    #获取当前项目的所有人  为下面发邮件准备数据
        project_users,principal = utils.project_members(db=app.config['cursor'])   #获取当前项目的所有人  为下面发邮件准备数据

        '''sendemail'''
        smtp_to = [(x+'@yuanxin-inc.com') for x in project_users]
        send_info = '创建%s项目成功,工作愉快..............'   % data['name']
        # utils.sendmail(app.config, smtp_to, send_info,send_info)

        utils.write_log(username,{'code':0,'result':'create project %s success'  %  data['name']})
...
def git_update(auth_info,**kwargs):
    ...
    try:
        ...
        if not result:
	    return json.dumps({'code':1, 'errmsg':'project is not exist'})    
        app.config['cursor'].execute_update_sql('git', data, where)
           
        r = gitolite()
        if r['code'] == 1:
            return json.dumps(r)
        utils.write_log(username,'update project %s success' % result['name'])
...
def git_get(auth_info, **kwargs):
    ...
    try:
	    ...
	    return json.dumps({'code':1, 'errmsg':'project is not exist'})
            ...
            utils.write_log(username, 'get project success ') 
...
def git_getlist(auth_info, **kwargs):
    ...
    try:
        ...
        for p  in projects:  # 循环项目列表，判断项目表中的p_user的id是否存在，如果存在则id2name
            ...
            p['p_group'] =  ','.join([groups[r] for r in p['p_group'].split(',')  if r in groups])
        
        # 普通用户只能查看其有权限的项目,上面总结果集projects中过滤
        if role != 0:  
	    p = utils.user_projects(username)  # 查出用户的项目id2name
            projects = [pro for pro in projects for pid  in p.keys() if pid==pro['id']] # 获取项目的详情  
        utils.write_log(username, '查询项目列表成功')  
...
def gitolite():
    ...
    group  = utils.group_members(db=app.config['cursor'])    # 获取所有组的成员信息
    project_users,principal = utils.project_members(db=app.config['cursor'])  # 获取所有项目的成员及负责人
    ...
    try:
        ...
        '''
        res, msg=utils.run_script_with_timeout("%s/git.sh" % app.config['script_path'])
        if res:
            return {'code':0,'result':"git操作成功"}
        else:
